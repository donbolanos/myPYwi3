...
class Colp :
    ...
    def __init__(self,
        ...
        if vlines is not None :
          try :
            ...
            raise TypeError("what the fucks you are plotting should be a vector!")
        else:
          self.vlines =  None
    ...
    def Draw(self):
        ...
        # .. draw vertical lines to represent lineout
        if self.vlines is not None : 
           for xline in self.vlines :
              ax.axvline(x=xline, ls = '--',color = 'k' ,lw=1.5)
        ...
        # .. set the colorbar
        cbar = fig.colorbar(im, ticks = ticks, pad = 0.03, aspect = 40, format = '$%1.02f$')
        if self.labelcb is not None : 
          cbar.set_label(self.labelcb)
          
        # .. draw the plot
        plt.tight_layout()
        if self.filetype is not None :
          fig.savefig(self.filename+'.'+self.filetype.lower(),
                      dpi = 200,
                      bbox_inches = 'tight')
