...
In order to see any variation of the outflow 
'''
#
import sys
import os
...
path = '/media/sbolanos/BatDRIVE/HECKLE/400x600_te/45deg_T100/'
name = 'laser'
...
for folder in liste :
  ...
  if folder[0:len(name)] == name :
        ...
        for time in np.arange(0,50.0) :
           if not time in t :
            t.append(time)
            # V : velocity 
            dataVx = run.GetV(time,'i')[:,ymoy,0]
            dataVy = run.GetV(time,'i')[:,ymoy,1]
            dataVz = run.GetV(time,'i')[:,ymoy,2]
